...
class DipPdfGenerator {
  ...
  async listen() {
    ...
    await this._amqp.consume(policyCreateQ.queue, this.issueCertificate.bind(this), { noAck: true });
  }
  ...
  async issueCertificate(message) {
    ...
    this._amqp.publish(shared.exhanges.policy, 'policy.certificate_issued.v1', Buffer.from(JSON.stringify({ policyId: message.properties.correlationId})), {
