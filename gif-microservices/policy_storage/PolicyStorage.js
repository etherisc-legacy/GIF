...
class PolicyStorage {
  ...
  async handleGetArtifact({ content, fields, properties }) {
    try {
      ...
      const product = content.product || properties.headers.product;
      const networkName = content.networkName || process.env.NETWORK_NAME;
      ...
      const response = await this.getArtifact(product, networkName, contractName);
